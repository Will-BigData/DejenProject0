...
class AirportManager:
    def __init__(self, airports=None):
        ...
        # Check if the airport with the given code exists
    ...
    def add_airport(self, code, name, city, country):
        # Add an airport if the code does not exist yet
    ...
    def view_airports(self):
        # Return all airports or a message if the list is empty
    ...
    def view_airport_by_code(self, code):
        # Corrected: Return the airport if the code matches, otherwise show "No airport found"
        for airport in self.airports:
            if airport["Airport Code"] == code:
                return airport
    ...
    def search_airports_by_name(self, name):
        # Search airports by partial name (case-insensitive)
